...
// **CORRECCIÓNn**: Se eliminaron todas las importaciones de Firebase que ya no se usan en el frontend.
...
function App() {
    ...
    useEffect(() => {
        ...
        const token = params.get('token'); const error = params.get('error');
        if (error) { setModal({ show: true, text: `Error de autenticación: ${error.replace(/_/g, ' ')}` }); setLoading(false); window.history.replaceState({}, document.title, "/login"); return; }
        if (token) { signInWithCustomToken(auth, token).catch(err => { console.error("Token Error:", err); setModal({ show: true, text: "Token de inicio de sesión inválido." }); }).finally(() => window.history.replaceState({}, document.title, "/")); }
        const unsubscribeAuth = onAuthStateChanged(auth, u => { setUser(u); setLoading(false); if (!u) { setSelectedGuild(null); setGuilds([]); }});
    ...
    if (!selectedGuild) return <ServerSelectionScreen guilds={guilds} user={user} onSelectGuild={handleSelectGuild} onLogout={handleLogout} loading={loadingGuilds} onRefresh={handleRefreshGuilds} />;
